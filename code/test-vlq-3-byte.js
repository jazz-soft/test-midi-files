...
smf.push(trk);
trk.tick(tt).smfSeqName('3-Byte VLQ')
  .smfCopyright('https://jazz-soft.net')
  .smfText('This file has 3-byte VLQ delta times. Technically, it\'s a valid MIDI file.\n')
...
var data = smf.dump();
while (data.indexOf('\xf0\x80') != -1) {
  data = data.replace('\xf0\x80', '\x80\x80');
}
